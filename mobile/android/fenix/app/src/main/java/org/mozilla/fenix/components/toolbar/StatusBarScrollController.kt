...
internal class StatusBarScrollController(
    ...
    fun synchronize(toolbar: View, topInset: Int, shouldSynchronize: Boolean) {
        val behavior = (toolbar.layoutParams as? CoordinatorLayout.LayoutParams)?.behavior
        if (!shouldSynchronize || !toolbar.isShown || behavior !is EngineViewScrollingBehavior) {
            enabled = false
            cancel()
            return
        }
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.R || topInset <= 0 || !stateChanged) {
            return
        }
